...
func (b *metricsBucket) snapshot() stageStats {
...
func newScenarioMetrics(s scenarioConfig) *scenarioMetrics {
...
func (m *scenarioMetrics) snapshot() scenarioStatsItem {
	stageStatsMap := make(map[string]stageStats)
...
func newRunStatsCollector(startedAt time.Time, scenarios []scenarioConfig) *runStatsCollector {
...
func (c *runStatsCollector) beginScenario(id string) {
	c.overall.begin()
	if item := c.scenario(id); item != nil {
		item.overall.begin()
	}
}

func (c *runStatsCollector) finishScenario(id string, status int, latency int64, success bool, timeout bool, errorKind string) {
...
func (c *runStatsCollector) beginStage(id string, stage string) {
	if item := c.scenario(id); item != nil {
		item.stage(stage).begin()
	}
}

func (c *runStatsCollector) finishStage(id string, stage string, status int, latency int64, success bool, timeout bool, errorKind string) {
	if item := c.scenario(id); item != nil {
		item.stage(stage).finish(status, latency, success, timeout, errorKind)
	}
}

func (c *runStatsCollector) summary(runID string, runStatus string, finishedAt time.Time) summaryResponse {
	...
	return summaryResponse{
		...
		P95MS:          overall.P95MS,
		P99MS:          overall.P99MS,
		ActiveRequests: overall.Active,
		StatusCodes:    statusCodes,
		ErrorKinds:     overall.ErrorKinds,
	}
}

func (c *runStatsCollector) scenariosSnapshot() scenarioStatsResponse {
	c.mu.RLock()
	defer c.mu.RUnlock()
	items := make([]scenarioStatsItem, 0, len(c.scenarios))
	for _, scenario := range c.scenarios {
	...
	return scenarioStatsResponse{Items: items}
}
